...
def get_gemini_response(prompt: str, model_type: str = "flash", max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            logging.info(f"Sending request to {model_type} model (attempt {attempt + 1}/{max_retries})")
...
def validate_api_key() -> bool:
    ...
    try:
        ...
        if response and response.text:
            ...
            logging.error("API key validation failed: No response")
            return False
...
def get_model_info() -> dict:
    ...
    """
    return {
        "thinking": "gemini-2.0-flash-thinking-exp - Best for complex reasoning",
        "lite": "gemini-2.0-flash-lite - Fast and efficient",
        "flash": "gemini-2.0-flash - Balanced performance"
    }
...
if __name__ != "__main__":
    try:
        if not validate_api_key():
            logging.warning("API key validation failed, but continuing...")
    except Exception as e:
        logging.warning(f"Could not validate API key: {e}")